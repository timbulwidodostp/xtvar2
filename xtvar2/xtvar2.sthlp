...
{p2colset 5 20 22 2}{...}
{p2col :{manlink R xtvar2} {hline 2}}Panel vector autoregressive models (PVARs){p_end}
{p2colreset}{...}
...
{p2colreset}{...}
{p 4 6 2}
You must {cmd:xtset} your data before using {cmd: xtvar2}; see 
{helpb xtset:[XT] xtset}. {cmd:xtvar2} requires a balanced panel dat aset.
{p_end} 
...
{pstd}
{cmd:xtvar2} fits a multivariate panel regression of each dependent variable on lags of itself and on 
...
{pstd}
{cmd: xtvar2} estimates IRFs, reports IRFs in tables, and visualizes IRFs in graphs. See Lütkepohl
(2005) for the estimation of IRFs. You can choose between two types of IRFs:

{p2colset 8 65 44 2}{...}
{p2col:IRFs to one-standard deviation shocks} after {cmd:xtvar2}{p_end}
{p2col:IRFs to one-unit shocks} after {cmd:xtvar2}, {opt stirf}{p_end}
{p2colreset}{...}
...
{pstd}
{cmd: xtvar2} estimates FEVDs, reports FEVDs in tables, and visualizes FEVDs in graphs. See Lütkepohl 
(2005) for the estimation of FEVDs. You can choose between two types of FEVDs:

{p2colset 8 65 44 2}{...}
{p2col:FEVDs based on IRFs to one-standard deviation shocks}after {cmd:xtvar2}{p_end}
{p2col:FEVDs based on IRFs to one-unit shocks}after {cmd:xtvar2}, {opt stirf}{p_end}
{p2colreset}{...}
...
{pstd}
{cmd: xtvar2} computes confidence intervals for IRFs and FEVDs. {cmd: xtvar2} computes confidence 
...
{pstd}
{cmd:xtvar2} either computes standard percentile intervals (default) or standard normal confidence 
...
{phang}
	{opt pooled} specifies that {cmd: xtvar2} estimates a multivariate least squares model with 
	common intercept for cross-sectional units. 

{dlgtab:Impulse Response Function}

{phang}
	{opt stirf} specifies that {cmd: xtvar2} estimates IRFs to one-unit shocks and 	FEVDs based 
	on IRFs to one-unit shocks. If {opt stirf} is not specified {cmd: xtvar2} estimates IRFs to 
...
{phang}
	{opt bsn} specifies that {cmd: xtvar2} estimates confidence intervals for IRFs and FEVDs 
	using the nonparametric residual bootstrap algorithm with the 
	{help xtvar2##temporal:{bf:temporal resampling scheme}}. You have to specify one of the 
	following options: {opt bsn}, {opt dbsn}, {opt bsp}, or {opt bsn}. 
		
{phang}
	{opt dbsn} specifies that {cmd: xtvar2} estimates confidence intervals for IRFs and FEVDs 
	using the nonparametric residual bootstrap algorithm with 
	{help xtvar2##double:{bf:double resampling scheme}}. You have to specify one of the 
	following options: {opt bsn}, {opt dbsn}, {opt bsp}, or {opt bsn}. 
	
{phang}
    	{opt bsp} specifies that {cmd: xtvar2} estimates confidence intervals for IRFs and FEVDs 
...
{phang}
	{opt mc} specifies that {cmd: xtvar2} estimates confidence intervals for IRFs and FEVDs 
	using the Monte-Carlo simulation algorithm.
	You have to specify one of the following options: {opt bsn}, {opt dbsn}, {opt bsp}, or 
	{opt bsn}. 

{phang}
	{opt norm} specifies that {cmd: xtvar2} computes standard normal confidence intervals instead 
...
{pstd}
{cmd:xtvar2} saves the following in {cmd:e()}:
...
{synopt:{cmd:e(cmd)}}{cmd:xtvar2}{p_end}
...
{it: Panel Vector Autoregressions for Stata (xtvar2).}
